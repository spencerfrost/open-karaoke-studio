  ...
  useEffect(() => {
    if (playerState.status === 'playing') {
      ...
      if (playerState.status === 'paused') {
        const pausedData = Array.from({ length: barCount }, (_, i) => 
      ...
      {audioData.map((value, index) => {
        ...
        return (
          <div
            ...
            style={{
              height: `${barHeight}px`,
              backgroundColor: index % 2 === 0 ? colors.darkCyan : colors.orangePeel,
              opacity: 0.7,
              transform: `translateY(${(height - barHeight) / 2}px)`,
              transition: playerState.status === 'playing' ? 'none' : 'height 0.5s',
