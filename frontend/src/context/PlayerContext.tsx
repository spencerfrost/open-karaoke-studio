...
export const PlayerProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getCurrentLyric = () => {
    ...
    const current = currentLyrics.length > 0 
      ? currentLyrics.reduce((latest, lyric) => 
          lyric.time > latest.time ? lyric : latest, 
          { time: 0, text: "" }
        ) 
      : null;
  ...
  useEffect(() => {
    ...
    if (state.status === 'playing') {
      interval = setInterval(() => {
        dispatch({ type: 'SET_CURRENT_TIME', payload: state.currentTime + 0.1 });
      }, 100);
    }
