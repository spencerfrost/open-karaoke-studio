...
def ensure_library_exists():
    ...
    # W0718: Catch specific OSError for file operations
    except OSError as e:
...
def get_song_dir(input_path: Path) -> Path:
    ...
    # Assuming ensure_library_exists handles base dir errors separately
...
def get_original_path(song_dir: Path, original_input_path: Path) -> Path:
    ...
    # C0301: Fix long line by creating filename separately
...
def save_original_file(input_path: Path, song_dir: Path) -> Path:
    ...
    # W0718: Catch specific IOErrors (subclass of OSError) or keep general Exception
    # Let's keep Exception here as shutil.copyfile can raise various things
