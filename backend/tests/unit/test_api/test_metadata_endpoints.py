...
class TestMetadataEndpoints:
    ...
    def test_search_endpoint_with_album(self, mock_metadata_service_class, client):
        ...
        response = client.get('/api/metadata/search?artist=Test Artist&title=Test Song&album=Test Album')
    ...
    def test_search_endpoint_empty_results(self, mock_metadata_service_class, client):
        ...
        response = client.get('/api/metadata/search?artist=Unknown Artist&title=Unknown Song')
    ...
    @patch('app.api.metadata.MetadataService')
    def test_search_endpoint_validation_error(self, mock_metadata_service_class, client):
        """Test metadata search with validation error"""
        # Mock the service instance and its methods
        mock_service = Mock()
        mock_metadata_service_class.return_value = mock_service
        mock_service.search_metadata.side_effect = ValueError("Invalid search parameters")
    ...
    def test_search_endpoint_server_error(self, mock_metadata_service_class, client):
        ...
        mock_service = Mock()
        mock_metadata_service_class.return_value = mock_service
        mock_service.search_metadata.side_effect = Exception("Network connection failed")
    ...
    def test_search_endpoint_missing_required_params(self, client):
        ...
        data = response.get_json()
        assert "error" in data
        assert "At least one of 'title' or 'artist' parameters is required" in data["error"]
    ...
    def test_search_endpoint_with_limit(self, mock_metadata_service_class, client):
        ...
        response = client.get('/api/metadata/search?artist=Test Artist&title=Test Song&limit=10')
    ...
    @patch('app.api.metadata.MetadataService')
    def test_search_endpoint_with_sort_by_param(self, mock_metadata_service_class, client):
        ...
        response = client.get('/api/metadata/search?artist=Test Artist&title=Test Song&sort_by=date_desc')
