...
from .models import SessionLocal, KaraokeQueueItem
...
def add_to_queue():
    ...
    session = SessionLocal()
    try:
        max_position = session.query(KaraokeQueueItem.position).order_by(KaraokeQueueItem.position.desc()).first()
...
def reorder_queue():
    ...
    try:
        for item in data['queue']:
            queue_item = session.query(KaraokeQueueItem).filter(KaraokeQueueItem.id == item['id']).first()
            if queue_item:
