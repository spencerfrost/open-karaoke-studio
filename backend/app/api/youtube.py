    ...
    try:
        ...
        if original_file_path.exists():
            current_app.logger.info(
                f"Submitting audio processing task for song {song_id}"
            )
            try:
                job = Job(
                    id=song_id,
                ...
                current_app.logger.error(
                    f"Failed to submit processing task: {task_error}", exc_info=True
                )
        else:
            current_app.logger.error(
                f"Original audio file not found at {original_file_path}"
            )
        ...
        if lyrics_data is None or "error" in lyrics_data:
            current_app.logger.warning(
                f"Lyrics not found or error for {song_title} by {artist}: {lyrics_data.get('error') if lyrics_data else 'No data'}"
            )
        else:
        ...
        if lyrics or synced_lyrics:
            ...
            current_app.logger.warning(
                f"No lyrics found for {metadata['title']} by {metadata['artist']}"
            )
...
def parse_metadata_endpoint():
    ...
    try:
        ...
        return jsonify({"title": title, "artist": artist, "original": youtube_title})
    except Exception as e:
