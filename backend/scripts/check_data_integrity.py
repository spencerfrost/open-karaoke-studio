...
from app.db.database import get_all_songs
...
class DataIntegrityChecker:
    def __init__(self, karaoke_lib_path: Optional[str] = None):
        ...
        self.karaoke_lib_path = Path(karaoke_lib_path) if karaoke_lib_path else Path(self.config.LIBRARY_DIR)
    ...
    def check_required_files(self, song_id: str) -> Dict[str, any]:
        ...
        try:
            ...
            expected_files = {'original.mp3', 'vocals.mp3', 'instrumental.mp3', 'metadata.json'}
                ...
                'missing_directory_info': [
                    {
                        ...
                        'artist': db_songs_info.get(song_id, {}).get('artist', 'Unknown')
                    }
...
class ReportGenerator:
    ...
    def generate_markdown_report(data: Dict[str, any]) -> str:
        ...
        report.append(f"| Total Filesystem Directories | {summary['total_fs_directories']} |")
        ...
        if data['discrepancies']['missing_directories']:
            ...
            for song_info in data['discrepancies']['missing_directory_info']:
                ...
                title = song_info['title'].replace('|', '\\|')  # Escape pipes for markdown
                artist = song_info['artist'].replace('|', '\\|')  # Escape pipes for markdown
                report.append(f"| `{song_id}` | {title} | {artist} |")
            report.append("")
        
        if data['discrepancies']['orphaned_directories']:
            report.append("## ⚠️ Orphaned Directories")
            report.append("Filesystem directories without corresponding database entries:")
            report.append("")
        ...
        report.append("| Song ID | Title | Artist | Original | Vocals | Instrumental | Thumbnail | Thumbnail Format | Metadata | Missing Files | File Sizes | Extra Files |")
        report.append("|---------|-------|--------|----------|--------|-------------|-----------|------------------|----------|---------------|------------|-------------|")
        
        for check in data['file_integrity']:
            song_id = check['song_id']
            title = check.get('title', 'Unknown').replace('|', '\\|')  # Escape pipes for markdown
            artist = check.get('artist', 'Unknown').replace('|', '\\|')  # Escape pipes for markdown
            ...
            missing_files = ", ".join(check['missing_files']) if check['missing_files'] else "None"
            extra_files = ", ".join(check['extra_files']) if check['extra_files'] else "None"
            ...
            report.append(f"| `{song_id}` | {title} | {artist} | {original} | {vocals} | {instrumental} | {thumbnail} | {thumbnail_format} | {metadata} | {missing_files} | {file_sizes} | {extra_files} |")
        ...
        if total_checks > 0:
            complete_songs = sum(1 for check in data['file_integrity'] 
                               if check['original_mp3'] and check['vocals_mp3'] 
                               and check['instrumental_mp3'] and check['thumbnail'])
            
            report.append("## Statistics")
            report.append("")
            report.append("| Metric | Count | Percentage |")
            report.append("|--------|-------|------------|")
            report.append(f"| Complete Songs (all 4 files) | {complete_songs} | {complete_songs/total_checks*100:.1f}% |")
            
            # File type statistics
            for file_type in ['original_mp3', 'vocals_mp3', 'instrumental_mp3', 'thumbnail']:
    ...
    def generate_csv_report(data: Dict[str, any]) -> str:
        ...
        fieldnames = [
            'song_id', 'title', 'artist', 'original_mp3', 'vocals_mp3', 'instrumental_mp3', 
            'thumbnail', 'thumbnail_format', 'metadata_json', 'missing_files', 
            'extra_files', 'file_sizes_mb'
        ]
        ...
        for check in data['file_integrity']:
            row = {
                ...
                'file_sizes_mb': '; '.join([f"{k}:{v//1024//1024}MB" if isinstance(v, int) and v > 0 else f"{k}:{v}" 
    ...
    def generate_text_report(data: Dict[str, any]) -> str:
        ...
        if total_checks > 0:
            ...
            report.append("-" * 25)
            report.append(f"Complete Songs (all 4 files): {complete_songs}/{total_checks} ({complete_songs/total_checks*100:.1f}%)")
            report.append("")
        
        return "\n".join(report)

def main():
    parser = argparse.ArgumentParser(description="Check data integrity between database and filesystem")
    parser.add_argument('--output', '-o', help='Save report to specified file')
    parser.add_argument('--format', '-f', choices=['markdown', 'json', 'csv', 'txt'], 
