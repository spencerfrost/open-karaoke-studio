...
from app.db.database import get_all_songs, get_db_session, delete_song
...
class CleanupItem:
    ...
    def __init__(self, item_type: str, song_id: str, reason: str, details: Dict = None):
...
class KaraokeLibraryCleanup:
    ...
    def identify_cleanup_items(self) -> List[CleanupItem]:
        ...
        for song_id in consistent_ids:
            ...
            essential_missing = [f for f in file_check['missing_files'] 
        ...
        print(f"   - {len([i for i in cleanup_items if i.item_type == 'missing_files'])} songs with missing MP3 files")
    ...
    def format_cleanup_item_info(self, item: CleanupItem) -> str:
        ...
        if item.item_type == 'orphaned_db':
            ...
            lines.append(f"🎵 Has original.mp3: {'✅' if item.details['has_original'] else '❌'}")
            lines.append(f"🎤 Has vocals.mp3: {'✅' if item.details['has_vocals'] else '❌'}")
            lines.append(f"🎼 Has instrumental.mp3: {'✅' if item.details['has_instrumental'] else '❌'}")
            lines.append(f"🖼️  Has thumbnail: {'✅' if item.details['has_thumbnail'] else '❌'}")
            if item.details['missing_files']:
                lines.append(f"❌ Missing: {', '.join(item.details['missing_files'])}")
            if item.details['extra_files']:
                lines.append(f"📄 Extra files: {', '.join(item.details['extra_files'])}")
    ...
    def prompt_user_for_deletion(self, item: CleanupItem) -> bool:
        ...
        while True:
            response = input("\n🗑️  Delete this item? (y/n/s/q) [y=yes, n=no, s=skip all remaining, q=quit]: ").lower().strip()
    ...
    def delete_cleanup_item(self, item: CleanupItem) -> bool:
        ...
        try:
            if item.item_type == 'orphaned_db':
                ...
                if self.dry_run:
                    print(f"🔄 [DRY RUN] Would delete database entry for song ID: {item.song_id}")
                else:
...
def main():
    parser = argparse.ArgumentParser(description="Clean up karaoke library by removing orphaned and problematic entries")
    ...
    parser.add_argument('--auto-confirm', action='store_true', help='Auto-confirm all deletions (dangerous!)')
