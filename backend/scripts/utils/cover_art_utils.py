...
def should_download_cover_art(song: DbSong, force_download: bool = False) -> tuple[bool, str]:
    """
    ...
    if is_low_resolution_cover_art(cover_art_path):
        if cover_art_path.exists():
            file_size = cover_art_path.stat().st_size
            return True, f"Low-res cover art ({file_size} bytes), will upgrade to high-res"
        else:
...
def download_high_res_cover_art(song: DbSong, itunes_data: Dict[str, Any], dry_run: bool = False) -> Optional[str]:
    """
    ...
    if relative_path:
        ...
        if cover_art_path.exists():
            ...
            if file_size > LOW_RES_THRESHOLD:
                logger.info(f"Successfully downloaded high-res cover art for song {song.id} ({file_size} bytes)")
                return relative_path
            else:
                logger.warning(f"Downloaded cover art for song {song.id} seems small ({file_size} bytes)")
                return relative_path
        else:
