...
from .cover_art_utils import download_high_res_cover_art, should_download_cover_art
from .database_utils import update_song_metadata
from .itunes_utils import enhance_song_with_itunes_data, search_itunes_for_song
...
class MetadataUpdater:
    ...
    def __init__(
        ...
        logger.info(
            f"MetadataUpdater initialized - dry_run: {dry_run}, batch_size: {batch_size}, delay: {delay}"
        )
    ...
    def process_song(self, song: DbSong, stats: ProcessingStats) -> bool:
        ...
        try:
            logger.debug(
                f"Processing song {song.id}: '{song.title}' by '{song.artist}'"
            )
            ...
            if not itunes_data:
                logger.info(
                    f"No iTunes match found for song {song.id}: '{song.title}' by '{song.artist}'"
                )
                return False
            ...
            should_download, reason = should_download_cover_art(
                song, self.force_cover_art
            )

            cover_art_path = None
            if should_download:
                logger.info(f"Song {song.id}: {reason}")
                cover_art_path = download_high_res_cover_art(
                    song, itunes_data, self.dry_run
                )
                if cover_art_path:
