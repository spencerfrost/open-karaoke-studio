...
def print_stats_summary():
    ...
    logger.info(f"With iTunes ID: {db_stats['with_itunes_id']} ({db_stats.get('with_itunes_id_percentage', 0):.1f}%)")
    logger.info(f"With cover art: {db_stats['with_cover_art']} ({db_stats.get('with_cover_art_percentage', 0):.1f}%)")
    logger.info(f"With genre: {db_stats['with_genre']} ({db_stats.get('with_genre_percentage', 0):.1f}%)")
    logger.info(f"With release date: {db_stats['with_release_date']} ({db_stats.get('with_release_date_percentage', 0):.1f}%)")
    logger.info(f"Unknown artist: {db_stats['unknown_artist']} ({db_stats.get('unknown_artist_percentage', 0):.1f}%)")
    ...
    total_needing_update = cover_stats['low_res_cover_art'] + cover_stats['without_cover_art'] + cover_stats['missing_files']
...
def main():
    ...
    parser.add_argument(
        "--stats-only", action="store_true", help="Only show statistics, don't process songs"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be updated without making changes"
    )
    parser.add_argument(
        "--limit", type=int, metavar="N", help="Only process first N songs (for testing)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force-cover-art", action="store_true", 
    ...
    try:
        songs = get_songs_to_process(limit=args.limit)  # Use args.limit (None for all songs)
        ...
        if songs:
            ...
            if args.dry_run:
                ...
                for i, song in enumerate(songs[:3], 1):  # Just process first 3 in dry-run
                    logger.info(f"  {i}. Would process: {song.title} by {song.artist}")
                    logger.info(f"     Current iTunes ID: {song.itunes_track_id or 'None'}")
                    logger.info(f"     Current cover art: {'Yes' if song.cover_art_path else 'No'}")
                ...
                try:
                    ...
                    logger.info(f"Created MetadataUpdater, processing {len(songs)} songs...")
                    ...
                    logger.info(f"Success rate: {(final_stats.updated / final_stats.processed * 100):.1f}%" if final_stats.processed > 0 else "N/A")
