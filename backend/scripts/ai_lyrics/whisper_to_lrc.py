...
class WhisperToLRCConverter:
    ...
    def _should_break_line(
        ...
        if (
            word_text.endswith(",")
            and len(current_line) >= self.target_words_per_line - 1
        ):
            return True
    ...
    def assess_quality(
        self, words: List[Dict[str, Any]], segments: List[Dict[str, Any]]
    ) -> QualityMetrics:
        """
        ...
        recommended_review = (
            avg_confidence < 0.6
            or low_confidence_words / len(words) > 0.3
            or coverage_percentage < 0.8
            or timing_consistency < 0.8
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python whisper_to_lrc.py vocals_whisper_results.json vocals.lrc"
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"🔍 Review recommended: {'Yes' if lyrics_data.quality.recommended_review else 'No'}"
        )
