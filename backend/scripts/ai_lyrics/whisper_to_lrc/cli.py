...
Examples:
  ...
  %(prog)s vocals_whisper_results.json --confidence-threshold 0.5 --quiet
    ...
    parser.add_argument(
        'input_file',
        help='Path to the Faster Whisper JSON output file'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help='Maximum words per line (default: 8)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Target words per line (default: 5)'
    )
    ...
    parser.add_argument(
        '--show-quality',
        action='store_true',
        help='Show detailed quality assessment'
    )

    parser.add_argument(
        '--show-metadata',
        action='store_true',
        help='Show conversion metadata'
    )
...
def validate_arguments(args: argparse.Namespace) -> bool:
    ...
    if args.target_words < args.min_words or args.target_words > args.max_words:
        print("❌ Error: target-words must be between min-words and max-words", file=sys.stderr)
        return False
...
def run_conversion(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.show_quality:
            ...
            print(f"  Low confidence words: {quality.low_confidence_words}/{quality.total_words}")
            print(f"  Coverage percentage: {quality.coverage_percentage:.1%}")
            print(f"  Timing consistency: {quality.timing_consistency:.3f}")
            print(f"  Review recommended: {'Yes' if quality.recommended_review else 'No'}")
        ...
        if args.show_metadata:
            ...
            if metadata.get('detected_language'):
                print(f"  Language: {metadata['detected_language']} ({metadata.get('language_probability', 0):.2f})")
            if metadata.get('duration'):
                print(f"  Duration: {metadata['duration']:.1f} seconds")
            print(f"  Words: {metadata['original_word_count']} total, {metadata['filtered_word_count']} used")
