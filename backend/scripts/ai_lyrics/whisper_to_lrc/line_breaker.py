...
from .models import LyricLine, ConverterConfig
...
class LineBreaker:
    ...
    def _should_break_line(self, 
        ...
        if word_text.endswith(',') and len(current_line) >= self.config.target_words_per_line - 1:
            return True
...
def detect_natural_breaks(words: List[Dict[str, Any]],
                         max_silence_gap: float = 1.5) -> List[int]:
    """
