...
from .models import LyricsData, ConverterConfig
...
def convert_whisper_dict_to_lrc(json_data: Dict[str, Any],
                               **converter_kwargs) -> Tuple[str, LyricsData]:
    """
...
def batch_convert_directory(input_dir: str,
                           output_dir: str,
                           pattern: str = "*.json",
                           **converter_kwargs) -> Dict[str, bool]:
    """
    ...
    for json_file in json_files:
        try:
            ...
            print(f"✅ {json_file.name} -> {lrc_file.name} ({len(lyrics_data.lines)} lines)")
...
def validate_lrc_output(lrc_content: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'avg_line_gap': (max(timestamps) - min(timestamps)) / len(timestamps) if len(timestamps) > 1 else 0,
