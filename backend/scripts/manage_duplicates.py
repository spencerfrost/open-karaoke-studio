...
from app.db.database import get_all_songs, delete_song
...
class DuplicateManager:
    ...
    def find_duplicate_groups(self) -> List[List[Dict]]:
        ...
        for song in songs:
            song_dict = {
                ...
                'normalized_title': self.normalize_title(song.title or ''),
                'original_song': song
            }
        ...
        for i, song1 in enumerate(song_data):
            ...
            if len(similar_group) > 1:
                ...
                is_exact_group = all(self.is_exact_duplicate(similar_group[0], song) 
                                   for song in similar_group[1:])
                duplicate_groups.append({
                    'songs': similar_group,
                    'is_exact': is_exact_group
                })
    ...
    def calculate_completeness_score(self, integrity_info: Dict) -> Tuple[int, int]:
        ...
        score = 0
        max_score = 4  # original, vocals, instrumental, thumbnail (metadata.json is legacy)
    ...
    def format_song_info(self, song: Dict, integrity_info: Dict) -> str:
        ...
        if integrity_info['extra_files']:
            status_indicators.append(f"📁 Extra files: {', '.join(integrity_info['extra_files'])}")
        
        status_text = "\n    ".join(status_indicators) if status_indicators else "✅ All files present"
  ...
  Issues: {status_text}"""
            ...
            if choice == 'q':
                ...
                try:
                    ...
                    if 0 <= keep_num < len(songs_with_info):
                        ...
                        print(f"Invalid song number. Please choose 1-{len(songs_with_info)}")
                        continue
                except ValueError:
                ...
                try:
                    ...
                    for num in delete_nums:
                        if 1 <= num <= len(songs_with_info):
                            ...
                            print(f"Invalid song number: {num}. Please choose 1-{len(songs_with_info)}")
                            break
                    else:
                        # All numbers are valid
                        to_delete = [songs_with_info[i]['song']['id'] for i in valid_nums]
                        return to_delete
                        
                except ValueError:
                    print("Invalid format. Enter song numbers separated by commas or spaces.")
                    continue
    ...
    def export_duplicate_report(self, duplicate_groups: List[List[Dict]], filename: str) -> None:
    ...
    def run_duplicate_management(self, dry_run: bool = False, auto_mode: bool = False, export_file: Optional[str] = None) -> None:
        ...
        for i, group in enumerate(duplicate_groups):
            songs_with_info = self.display_duplicate_group(group, i + 1)
            ...
            if to_delete and not auto_mode:
                ...
                for song_info in songs_with_info:
                    if song_info['song']['id'] in to_delete:
                        songs_to_delete_names.append(f"{song_info['song']['title']} by {song_info['song']['artist']}")
            ...
            print(f"✅ Successfully deleted {deleted_count}/{len(to_delete)} songs from this group")
...
def main():
    parser = argparse.ArgumentParser(description="Manage duplicate songs in Open Karaoke Studio")
    ...
    parser.add_argument('--export-report', help='Export duplicate groups to a file before processing')
